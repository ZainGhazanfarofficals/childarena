					...
					$ratings = '';
		  			if ( class_exists('WCVendors_Pro') && !WCVendors_Pro::get_option( 'ratings_management_cap' ) ) {
		  				...
		  				<div class="seller-info-rating">
			  				...
			  				if ( $average_rate !=0 ) {
				  				echo sprintf(_n('Rating: <strong>%s</strong> based on %d rating', 'Rating: %s based on %d ratings', $rate_count, 'kossy'), $average_rate, $rate_count);
				  				echo '<a href="'.esc_url($url).'" class="btn">'.esc_html__('View all ratings', 'kossy').'</a>';
			  				} else {
			  					echo esc_html__("Rating: This Seller still doesn't have any ratings yet.", 'kossy');
			  				}
			  				?>
		  				</div>
		  				<?php
