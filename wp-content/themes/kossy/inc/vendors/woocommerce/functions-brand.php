...
if ( !class_exists("Kossy_Woo_Brand") ) {
	class Kossy_Woo_Brand {

		public static function init() {
			// brand
			$tax = kossy_get_config( 'product_brand_attribute' );
			if ( !empty($tax) ) {
		...
		public static function add_brand() {
			...
			<div class="form-field">
				<label><?php esc_html_e( 'Thumbnail', 'kossy' ); ?></label>
		...
		public static function edit_brand( $term ) {
			...
			?>
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'kossy' ); ?></label></th>
				<td>
		...
		public static function brand_columns( $columns ) {
			...
			foreach ($columns as $key => $value) {
				if ( $key == 'name' ) {
					$new_columns['image'] = esc_html__( 'Image', 'kossy' );
				}
		...
		public static function brand_column( $columns, $column, $id ) {
			if ( $column == 'image' ) {
				...
				$columns .= '<img style="max-width: 60px;" src="' . esc_url( $image ) . '" alt="'.esc_attr__( 'Image', 'kossy' ).'" class="wp-post-image" />';
			}
		...
		public static function get_product_brands() {
		    global $product;
		    $brands_tax = kossy_get_config( 'product_brand_attribute' );
	...
	add_action( 'init', array('Kossy_Woo_Brand', 'init') );
}