...
<div class="widget-blogs widget no-margin <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($el_class); ?>">
    <?php if ($title!=''): ?>
    ...
    <div class="widget-content">
        ...
        <?php elseif ( $layout_type == 'grid' ): ?>
            ...
            <div class="layout-blog style-grid">
                <div class="row">
                    <?php $i=0; while ( $loop->have_posts() ) : $loop->the_post(); ?>
                        <div class="col-md-<?php echo esc_attr($bcol); ?> col-sm-6 col-xs-12 <?php echo esc_attr($classes); ?>">
                            <?php get_template_part( 'template-posts/loop/inner-grid-v2' ); ?>
                        </div>
                    <?php $i++; endwhile; ?>
                </div>
    ...
    <?php if ( isset($show_pagination) && $show_pagination && $layout_type != 'carousel' ): ?>
        <?php kossy_pagination( $posts_per_page, $loop->found_posts, $loop->max_num_pages ); ?>
    <?php endif ; ?>
</div>
